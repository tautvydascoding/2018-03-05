<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <title>MAAAATRICA</title>
        <!-- reikalinga kad prisitaikantis dizainas veiktu tvarkingai -->
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <link rel="stylesheet" href="libs/bootstrap4/css/bootstrap.min.css">
        <!-- galimos klaidos -->
        <!-- blogas kelias iki failo -->
        <!-- "/" ne i ta puse -->
        <!-- neuzdarete ">" -->
        <link rel="stylesheet" href="css/master.css">
        <!-- !!!! VISSA MANO CSS failas pats zemiausias -->
    </head>

    <body>



<?php
      $darbuotojai = [];
      $x1 = ['Tomas', 1981, "Buhalteris"];
      $x2 = ['Ona', 1980, "Programuotoja"];
      $x3 = ['Petras', 1975, "Direktorius"];

      $darbuotojai[0] = $x1;
      $darbuotojai[1] = $x2;
      $darbuotojai[2] = $x3;

echo "Tomo specialybe yra: " . $darbuotojai[0][2] . "<br />";
echo "Petro gimimo data yra : " . $darbuotojai[2][1] . "<br />";



      for ($i=0; $i < count($darbuotojai); $i++){
        echo "Vardas: " . $darbuotojai[$i][0] . "<br />";
        echo "Metai: " . $darbuotojai[$i][1] . "<br />";
        echo "Pareigos: " . $darbuotojai[$i][2] . "<br />";
      }

  // FOR FOR
    for ($i=0; $i < count($darbuotojai); $i++) { // count)darb..) - kiek skirtingu darbuotoju
      for ($g=0; $g < count($darbuotojai); $g++) { //count(darb..) - Vardas, data, pareigos
        echo "FORFOR " . $darbuotojai[$i][$g] . "<br />";
      }
    }



//klausimai teste ====================
for ($i=0; $i <2 ; $i++) {
  for ($g=0; $g < 5; $g++) {  //kiek kartu isves labas
    echo "Labas: <br />";     //ATS 10
  }
}

echo "$g <br />";
echo "$i <br />"; // kiek bus g ir i ? g=5, i=2


for ($i=0; $i <3 ; $i++){
echo "Pasiruosim : <br /> ";
  for ($g=0; $g <3 ; $g++) {
    echo "dirbu: <br /> ";
  }
}


// 1. G??
// 2. I ??
// 3. KIEK KARTU DIRBU ATSPAUSD ??
// 4. kiek kartu pasiruosim dyrbt ??




 ?>










        <!-- bootstrap4 -->
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
        <!-- !!! jQuery "slim" yra nepilna versija ir jQuery kai kurios komandos neveikia, reiktu isisdeti kita jQuery  -->
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
        <!-- !!! mano js failas visada zemiausias -->
        <script type="text/javascript" src="main.js"> </script>
    </body>
</html>
