...
function updateDoctor($nr, $vardas, $pavarde) {
    ...
    if ( !$arPavyko ) {
         // '(! )' JEIGU skliaustuose FALSE?
         echo "EROROR: nepavyko pakeisti gydytojo pavardes." . mysqli_error( getDBPrisiungimas() );
    } else {
        echo "pavyko pakeisti";
    }
}
  // updateDoctor(3, 'Ona', 'Hardkauskiene');
...
function getDoctors($kiekis = 9999) {
  $mano_sql_tekstas = "SELECT * FROM doctors
                      ...
                      LIMIT $kiekis
  ";
  // LIMIT reiksme rasome ne kabutese
  $rezultatai = mysqli_query(getDBPrisiungimas(), $mano_sql_tekstas);
  // tikrinu ar gryzo duomenu
  if ($rezultatai) {
    return $rezultatai; // musql objektas
  } else {
    return NULL; //
  }
}
  // testuojame ar veikia
  $gydytojai = getDoctors(); // skaicius - kiek gydytoju paimsime
  $vienas_gydytojas = mysqli_fetch_assoc($gydytojai); //mysqli_fetch_assoc - paima sekanti gydytoja is mysql objekto IR sudejo jo stupelius i ARRAY
  // tikrinu ar turiu gydytojo duomenis
  while ($vienas_gydytojas) {
    echo "<h2>" . $vienas_gydytojas['name'] . $vienas_gydytojas['lname'] . "</h2>";
    $vienas_gydytojas = mysqli_fetch_assoc($gydytojai); // mysqli_fetch_assoc - paima sekanti gydytoja is mysql objekto IR sudeja jo stulpelius i ARRAY
  }
